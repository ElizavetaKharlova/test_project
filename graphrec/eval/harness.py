...
NDCG@K, and catalog coverage. PageRank is the per-user bottleneck, so evaluation
runs on a reproducible sample of users by default (``n_users``); pass 0 for all.
"""
...
def evaluate(
    ...
    *,
    ...
    using the full feature schema is added alongside the bipartite one.
    """
    ...
    if n_users and len(eligible) > n_users:
        eval_users = [int(u) for u in rng.choice(eligible, size=n_users, replace=False)]
    else:
        eval_users = eligible
    ...
    for rec in recommenders:
        rec.fit(train)
        recalls, ndcgs, recommended_union = [], [], set()
